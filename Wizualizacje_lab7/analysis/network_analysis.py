...
def compare_projections(datatype, model_name, n_layer, label, size=2000):
    ...
    # plot_new_neuron_projection(bt_etc, hue=bt_etc.feature_importances_)
    # plot_new_neuron_projection(at_etc, hue=at_etc.feature_importances_)
...
def show_seq_projections(datatype, model_name, n_layer, epoch, size):
    ...
    if "mlp" in model_name:
        ...
        # tak było oryginalnie i nie dzialalo :((
        # layer_at = get_activations_cnn(model_at, X_test, size)[n_layer - 1]
        layer_at = get_activations_cnn(model_at, X_test, size)[n_layer - 2]
    ...
    points_transformed, targets = show_tsne(model_name, epoch, layer_at, Y_test, init=init)
